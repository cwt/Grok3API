...
Метод `GrokClient.send_message` принимает множество параметров для настройки вашего запроса. Вот пример с настройками:
...
Метод `send_message` возвращает объект `GrokResponse`. Вот пример работы с ним:
