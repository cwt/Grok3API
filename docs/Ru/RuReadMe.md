...
#### ✨ Новое:
- 💬 **Поддержка продолжения бесед с Grok** — `client.ask` теперь использует `conversation_id` и `response_id` для продолжения бесед. Решение [issue](https://github.com/boykopovar/Grok3API/issues/4)
- ➕ **Новые параметры управления беседами**:
  - `always_new_conversation` — всегда новый чат.
  - `new_conversation` — новый чат при отправке запроса.
- 🆙 **Расширен объект `GrokResponse`** — добавлены новые поля: `conversationId`, `title`, `conversationCreateTime`, `conversationModifyTime`, `temporary`.



#### 📋 Примечания:
- ✅ Чаты теперь сохраняются на серверах Grok и подгружаются при запросах.
- ⚠️ Чаты с другими cookies не загружаются.
